 ...
 */
function _cmpFooters(a: Row, b: Row, options?: StripFootersOptions): boolean {
 ...
 * @param options
 */
export function stripFooters(rows: Row[], options?: StripFootersOptions): Row[] {
    ...
    const m = footers.map((ri, i) => footers.map((rj, j) =>
        (j < i) ? _cmpFooters(ri, rj, options) : undefined
    ));
